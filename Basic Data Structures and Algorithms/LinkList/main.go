...
func InsertHeroNodeByNoAse(headNode *HeroNode, newHeroNode *HeroNode) {
	//1,找到适当的节点
	//2,创建一个辅助节点
	temp := headNode
	flag := true
...
func main() {
	...
	//加入
	InsertHeroNodeByNoAse(&head, &hero2)
	InsertHeroNodeByNoAse(&head, &hero1)
	InsertHeroNodeByNoAse(&head, &hero3)
	//显示
	ListHeroNode(&head)
	fmt.Println()
	//删除
